...
`flowd` is still in pre-release / MVP stage, but v0.2 makes the project notably more usable as a local-first, terminal-first workflow engine. This release expands the open-core pipeline with better suggestion ranking, clearer explainability, richer local feedback and usage signals, easier setup, and more ways to observe repeated workflows without giving up deterministic behavior.
...
- Improved suggestion ranking and CLI display so repeated workflows surface more clearly and remain inspectable.
...
- Improved first-run setup and installer-oriented onboarding for faster local evaluation.
- Expanded observation with terminal history, clipboard, and browser downloads adapters.
- Added example workflow documentation to show the kinds of safe, deterministic automations `flowd` is designed to support.

## Core Workflow Engine

- Expanded the open-core engine to persist suggestion feedback history as part of local workflow state.
- Improved deterministic ranking and suggestion display so useful repeated patterns are surfaced more consistently.
...
- Added clearer terminal workflows for inspecting config, suggestions, history, and local stats.
...
- Added terminal history ingestion to capture repeated terminal-driven file workflows.
